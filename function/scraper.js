...
function ssweb (url, device = 'desktop') {
   ...
   function sholat(daerah) {
	return new Promise(async(resolve, reject) =>{
		axios.get('https://kalam.sindonews.com/jadwalsholat/' + daerah).then(({ data }) => {
			const $ = cheerio.load(data)
			const result = []
...
module.exports = { pinterest, wallpaper, wikimedia, quotesAnime, komiku, ssweb, sholat, tafsirsurah, fbdl }